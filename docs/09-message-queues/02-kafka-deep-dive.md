---
title: 2. Kafka Deep Dive
sidebar_position: 2
description: Master Apache Kafka - topics, partitions, replication, ISR, consumer groups, and delivery semantics for interviews.
keywords: [kafka, partitions, consumer groups, replication, isr, offsets, exactly once]
---
...
│                                                                      │
│ FOLLOWER:                                                            │
│   • Passive - just copies from leader                               │
...
Broker 1 (Leader)    Broker 2 (ISR)     Broker 3 (Out of Sync)
    Offset: 100          Offset: 99          Offset: 50
         ✓                   ✓                   ✗
    
A replica is IN-SYNC if:
• It has fetched all messages within replica.lag.time.max.ms (default 30s)
• Or within replica.lag.max.messages (deprecated)

WHY ISR MATTERS:
• acks=all → waits for ALL ISR replicas to acknowledge
• min.insync.replicas → minimum ISR required for writes
• If ISR < min.insync.replicas → writes are rejected
...
CONFIGURATION:
  ...
  • 1 broker down → ISR = 2, writes OK
  • 2 brokers down → ISR = 1, writes REJECTED (safety first)
```
...
```text
When leader fails:
1. Controller (special broker) detects failure
...
unclean.leader.election.enable = false (default)
  ...
  → May cause unavailability if all ISR down
  → NO data loss

unclean.leader.election.enable = true
  → Any replica can become leader
  → Always available
  → MAY LOSE DATA (out-of-sync replica missing messages)
```
...
**Next:** [3. RabbitMQ & AMQP →](./rabbitmq-amqp)
